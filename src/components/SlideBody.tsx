...
export function SlideBody({ slide, className = '' }: SlideBodyProps) {
  if (slide.body_html) {
    return (
      <div
        className={`slide-body-html leading-relaxed text-slate-100 [&_a]:border-b-2 [&_a]:border-sky-400 [&_a]:text-inherit [&_a]:no-underline [&_p+_p]:mt-6 [&_p]:leading-relaxed [&_strong]:font-semibold [&_strong]:text-white ${className}`}
  ...
  return (
    <p className={`whitespace-pre-wrap leading-relaxed text-slate-100 ${className}`}>
      {slide.body}
    </p>
  );
}
